...
function insertTestData() {
  ...
  try {
    ...
    // 插入教师休息日（Emma周日全天休息，John周六全天休息）
    const offDayInsert = db.prepare('INSERT INTO teacher_off_days (teacher_id, week_start_date, day_of_week, time_slot) VALUES (?, ?, ?, ?)');
    
    // Emma周日休息（所有时段）
    for (let slot = 1; slot <= 10; slot++) {
      offDayInsert.run(teacherIds[0], weekStart, 7, slot);
    }
    
    // John周六休息（所有时段）
    for (let slot = 1; slot <= 10; slot++) {
      offDayInsert.run(teacherIds[1], weekStart, 6, slot);
    }
