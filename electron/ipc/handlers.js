...
const studentDetailService = require('../services/studentDetailService');
const teacherDetailService = require('../services/teacherDetailService');
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('delete-student-teacher-subject', (event, studentId, subject) => 
    scheduleService.deleteStudentTeacherSubject(studentId, subject));
  ...
  // 学生可排课时段
  ipcMain.handle('get-student-available-times', (event, studentId, weekStartDate) => 
    studentAvailableTimeService.getStudentAvailableTimes(studentId, weekStartDate));
  ipcMain.handle('set-student-available-times', (event, studentId, weekStartDate, timesArray) => 
    studentAvailableTimeService.setStudentAvailableTimes(studentId, weekStartDate, timesArray));
  ...
  ipcMain.handle('delete-all-data', async () => {
    return await dataService.deleteAllData();
  });

  // 学员详细信息
  ipcMain.handle('get-student-details', (event, studentId) => 
    studentDetailService.getStudentDetails(studentId));
  ipcMain.handle('get-all-students-details', () => 
    studentDetailService.getAllStudentsDetails());
  ipcMain.handle('update-student-details', (event, studentId, details) => 
    studentDetailService.updateStudentDetails(studentId, details));
  ipcMain.handle('calculate-completed-hours', (event, studentId) => 
    studentDetailService.calculateCompletedHours(studentId));

  // 教师详细信息
  ipcMain.handle('get-teacher-details', (event, teacherId) => 
    teacherDetailService.getTeacherDetails(teacherId));
  ipcMain.handle('get-all-teachers-details', () => 
    teacherDetailService.getAllTeachersDetails());
  ipcMain.handle('update-teacher-details', (event, teacherId, details) => 
    teacherDetailService.updateTeacherDetails(teacherId, details));

  // 手动数据库迁移
  ipcMain.handle('run-manual-migration', () => 
    migrationService.runManualMigration());

  // 教室管理
  ipcMain.handle('get-classrooms', () => 
    classroomService.getAllClassrooms());
  ipcMain.handle('add-classroom', (event, roomNumber, status) => 
    classroomService.addClassroom(roomNumber, status));
  ipcMain.handle('update-classroom', (event, id, roomNumber, status) => 
    classroomService.updateClassroom(id, roomNumber, status));
  ipcMain.handle('delete-classroom', (event, id) => 
    classroomService.deleteClassroom(id));
  ipcMain.handle('update-classroom-status', (event, id, status) => 
