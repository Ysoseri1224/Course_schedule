...
function CourseArrangementModal({ visible, onClose, onSave, initialArrangements = [], studentId }) {
  ...
  }, [visible, studentId, initialArrangements]);
  ...
  const loadStudentSubjects = async () => {
    ...
    try {
      ...
      if (subjects.length === 0) {
        message.warning('该学生尚未设置任何科目教师，请先在“设置教师”中配置');
      }
    } catch (error) {
      message.error('加载学生科目失败');
      console.error(error);
    } finally {
      setLoading(false);
    }
