...
function StudentAvailableTimeModal({ visible, open, onClose, student }) {
  ...
  const loadAvailableTimes = async () => {
    ...
    try {
      ...
      const times = await window.api.getStudentAvailableTimes(student.id, weekStart);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await window.api.setStudentAvailableTimes(student.id, weekStart, timesArray);
  ...
  const getWeekRange = () => {
    ...
    const end = selectedWeek.add(6, 'day');
