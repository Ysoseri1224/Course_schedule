      ...
      {isAdmin && adminMenuExpanded && (
        <div className="border-t border-gray-200 bg-red-50 p-3 space-y-2">
          ...
          <button
            onClick={async () => {
              try {
                const results = await window.api.runManualMigration();
                const messages = [];
                if (results.students.success) messages.push(results.students.message);
                else messages.push('学员表: ' + results.students.message);
                if (results.teachers.success) messages.push(results.teachers.message);
                else messages.push('教师表: ' + results.teachers.message);
                
                if (results.students.success && results.teachers.success) {
                  message.success('数据库迁移成功！' + messages.join('\n'));
                } else {
                  message.warning('迁移部分完成：' + messages.join('\n'));
                }
              } catch (error) {
                message.error('数据库迁移失败：' + error.message);
              }
            }}
